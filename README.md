...
This preserves the existing frontend source and adds the WordPress plugin files, `templates/`, imported assets in `wp-app/`, and Composer metadata. Pass a target directory as the first argument when you are not running it from the app root:

The augmenter refuses to replace an existing asset directory unless it was generated by create-wp-app. Set `WP_APP_FRONTEND_ASSET_DIR` when your app already uses `wp-app/`.
